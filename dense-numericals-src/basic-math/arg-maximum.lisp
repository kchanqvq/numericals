...
(defpolymorph (nu:arg-maximum :inline t) ((x (array <type>))
  ...
  (pflet ((val (type-min <type>))
          ...
          (c-val-name  (c-name <type> 'nu:maximum))
          (c-size  (c-size <type>)))
    (declare (type <type> val))
    (cl:let ((acc     -1)
             (offset   0)
             (stride (reduce #'* (cdr (narray-dimensions x)))))
      (declare (cl:type (signed-byte 64) acc offset stride))
      (ptr-iterate-but-inner (narray-dimensions x)
          n
        ((ptr-x c-size inc-x x))
        (pflet ((new-val (inline-or-funcall c-val-name n ptr-x inc-x)))
          (declare (type <type> new-val))
          (when (> new-val val)
            (setq acc (+ offset (inline-or-funcall c-name n ptr-x inc-x))
                  val new-val)))
        (incf offset stride))
      acc)))
  ...
  (flet ((our-arg-maximum (array)
    ...
    (loop :for *array-element-type* :in `(single-float
          ...
          :do
             ...
             (5am:is (= 2 (nu:arg-maximum (make-view (nu:asarray '(1 2 3))))))
             (5am:is (= 8 (nu:arg-maximum (nu:asarray '((1 2 3)
                                                        (4 5 6)
                                                        (7 8 9))))))
             (5am:is (= 8 (nu:arg-maximum (make-view (nu:asarray '((1 2 3)
                                                                   (4 5 6)
                                                                   (7 8 9)))))))
